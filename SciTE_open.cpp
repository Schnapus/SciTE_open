...
{
    HWND target = 0;
    const auto SDI = RegisterWindowMessage(L"SciTEDirectorInterface");
    auto w = GetWindow(GetDesktopWindow(), GW_CHILD);
...
int main(int argc, char** argv)
{
    ...
    if (w == 0)
    {
        ...
        ShellExecute(0, L"open", L"scite.exe", NULL, L"c:\\", SW_SHOW);
    ...
    for (; it != end; ++it)
    {
        ...
        SendMessage(w, WM_COPYDATA, 0, (LPARAM)&cds);
    }
